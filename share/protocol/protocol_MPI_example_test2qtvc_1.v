...
Parameter p3: protocol.

Axiom Ensures2 : (p3 = (getNext p2)).

Parameter p4: protocol.

Axiom Ensures3 : (p4 = (simpl p3)).

(* Why3 goal *)
Theorem test2'vc :
  (p4 =
   (Seq (Message rank (rank + 10%Z)%Z MPI_int)
    (Foreach (rank + 1%Z)%Z 9%Z
     (fun (i:Numbers.BinNums.Z) => Message i (i + 10%Z)%Z MPI_int)))).
Proof.
rewrite Ensures3.
rewrite Ensures2.
rewrite Ensures1.
rewrite Ensures.
destruct (split'def (Foreach 0 9 (fun i : int => Message i (i + 10) MPI_int)) rank).
rewrite H0.
+ rewrite (getNext'def
     (Seq (Foreach 0 (rank - 1) (fun i : int => Message i (i + 10) MPI_int))
        (Foreach rank 9 (fun i : int => Message i (i + 10) MPI_int)))).
  destruct (simpl'def (Foreach rank 9 (fun i : int => Message i (i + 10) MPI_int))).
   rewrite H4.
   * reflexivity.
   * specialize H2. omega.
+ specialize H2. specialize H. omega.
Qed.
