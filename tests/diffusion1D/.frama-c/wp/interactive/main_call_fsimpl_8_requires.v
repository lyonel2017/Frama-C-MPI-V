...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (t4:addr -> Numbers.BinNums.Z) (i:Numbers.BinNums.Z) (c:S15_c_protocol)
    ...
    (c22:S15_c_protocol) (c23:S15_c_protocol)
    (t5:Numbers.BinNums.Z -> Numbers.BinNums.Z)
    (t6:Numbers.BinNums.Z -> Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (i2:Numbers.BinNums.Z),
  let a5 := L_get_type c7 in
  let a6 := L_get_type c3 in
  let a7 := L_get_type c10 in
  let a8 := getRight a7 in
  let a9 := L_get_type c in
  let a10 := getRight a9 in
  let x2 := t4 a in
  let a11 := L_get_type c21 in
  let x3 := t3 a1 in
  let a12 := getRight a5 in
  let a13 := getRight a6 in
  let a14 := L_get_type c19 in
  let a15 := getLeft a9 in
  let a16 := getLeft a7 in
  let a17 := split_right a15 (1%Z + i)%Z in
  let x4 := t2 a2 in
  let a18 := L_get_type c17 in
  let a19 := getRight a12 in
  let x5 := ZArith.BinInt.Z.quot x2 x in
  let a20 := L_get_type c22 in
  let a21 := L_get_type c20 in
  let a22 := L_get_type c18 in
  let a23 := L_get_type c15 in
  let a24 := L_get_type c12 in
  let a25 := L_get_type c5 in
  let a26 := L_get_type c4 in
  let a27 := L_get_type c11 in
  let a28 := getLeft a24 in
  let a29 := L_get_type c23 in
  let a30 := getLeft a29 in
  let a31 := L_get_type c14 in
  let a32 := havoc t4 t (shift a 0%Z) 1%Z in
  let a33 := L_to_list a32 a 0%Z 1%Z in
  let a34 := getLeft a11 in
  let a35 := havoc t3 a32 (shift a1 0%Z) 1%Z in
  let a36 := L_to_list a35 a1 0%Z 1%Z in
  let a37 := getLeft a14 in
  let a38 := havoc t2 a35 (shift a2 0%Z) 1%Z in
  let a39 := L_to_list a38 a2 0%Z 1%Z in
  let a40 := havoc t1 a38 a4 x5 in
  let a41 := getLeft a25 in
  let a42 :=
  L_to_list
  (map.Map.set (map.Map.set a40 (shift a3 0%Z) i2) (shift a3 (1%Z + x5)%Z)
   i1)
  a4 0%Z 1%Z in
  ...
  ((getLeft a19) = a17) -> ((getRight a19) = a10) -> (0%Z < rank)%Z ->
  (0%Z <= i)%Z -> (0%Z <= rank)%Z -> (i < x3)%Z -> (0%Z <= x3)%Z ->
  (i <= x3)%Z -> (0%Z <= x5)%Z -> is_sint32 i -> is_sint32 rank ->
  P_set_type c23 the_protocol -> size_constrain x -> is_sint32 x1 ->
  is_sint32 x -> is_sint32 x2 -> is_sint32 x3 -> is_sint32 x4 ->
  is_sint32 x5 -> P_set_type c21 (simpl a20) -> P_set_type c19 (simpl a21) ->
  P_set_type c17 (simpl a22) -> P_set_type c15 (assoc (L_get_type c16)) ->
  P_set_type c14 (getRight a23) -> P_set_type c12 (assoc (L_get_type c13)) ->
  P_set_type c11 (getRight a24) -> P_set_type c8 (assoc (L_get_type c9)) ->
  P_set_type c7 (assoc (L_get_type c8)) ->
  P_set_type c5 (assoc (L_get_type c6)) -> P_set_type c4 (getRight a25) ->
  P_set_type c3 (getRight a26) -> P_set_type c1 (assoc (L_get_type c2)) ->
  P_set_type c (getRight a27) -> isSkip (fsimpl (getLeft a23)) ->
  isSkip (fsimpl (getLeft a27)) -> isSkip (fsimpl (getLeft a26)) ->
  isMessageforIntRecv a28 0%Z x5 0%Z ->
  P_set_type c6 (Seq (simpl protocol_6) a12) ->
  isforIntBroadcast a30 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c13 (Seq (simpl (getLeft a31)) (getRight a31)) ->
  P_set_type c9 (Seq (simpl a16) a8) ->
  P_set_type c2 (Seq (split protocol_7 x1) a13) ->
  P_set_type c16 (Seq (split (getLeft a18) rank) (getRight a18)) ->
  predIntBroadcast a30 a33 ->
  isforIntBroadcast a34 0%Z 1%Z (L_to_list a32 a1 0%Z 1%Z) ->
  countiIntBroadcast a30 (getRight a29) a20 a33 ->
  predIntBroadcast a34 a36 ->
  isforIntBroadcast a37 0%Z 1%Z (L_to_list a35 a2 0%Z 1%Z) ->
  countiIntBroadcast a34 (getRight a11) a21 a36 ->
  predIntBroadcast a37 a39 ->
  countiIntBroadcast a37 (getRight a14) a22 a39 ->
  predIntMessage a28 (L_to_list a40 a4 0%Z x5) -> isPredIntMessage a41 a42 ->
  isMessageforIntSend a41 x1 1%Z 0%Z a42 ->
  ((0%Z < x5)%Z -> (x5 <= 10001%Z)%Z) ->
  (forall (i3:Numbers.BinNums.Z), (0%Z <= i3)%Z -> (i3 <= 10001%Z)%Z ->
   (i3 < 0%Z)%Z \/ (x5 <= i3)%Z -> ((t6 i3) = (t5 i3))) ->
  isSkip (fsimpl (getLeft (L_get_type c1))).
Proof.
intros x a a1 a2 x1 a3 a4 t t1 t2 t3 t4 i c c1 c2 c3 c4 c5 c6 c7 c8 c9 c10
c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 t5 t6 i1 i2 a5 a6 a7 a8
a9 a10 x2 a11 x3 a12 a13 a14 a15 a16 a17 x4 a18 a19 x5 a20 a21 a22 a23 a24
a25 a26 a27 a28 a29 a30 a31 a32 a33 a34 a35 a36 a37 a38 a39 a40 a41 a42 h1 h2
...
h62 h63 h64 h65 h66 h67 h68 h69 h70.
erewrite Q_link by apply h46.
erewrite Q_link by apply h56.
unfold protocol_7.
