...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (c26:S15_c_protocol) (t6:Numbers.BinNums.Z -> Numbers.BinNums.Z)
    (t7:Numbers.BinNums.Z -> Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (i2:Numbers.BinNums.Z),
  let a5 := L_get_type c10 in
  let a6 := L_get_type c13 in
  let a7 := getRight a6 in
  let a8 := L_get_type c in
  let a9 := getRight a8 in
  let x2 := t5 a in
  let a10 := L_get_type c24 in
  let x3 := t4 a1 in
  let a11 := getRight a5 in
  let a12 := L_get_type c22 in
  let a13 := getLeft a8 in
  let a14 := getLeft a6 in
  let x4 := t3 a2 in
  let a15 := L_get_type c20 in
  let a16 := getRight a11 in
  let x5 := ZArith.BinInt.Z.quot x2 size in
  let a17 := L_get_type c25 in
  let a18 := L_get_type c23 in
  let a19 := L_get_type c21 in
  let a20 := L_get_type c18 in
  let a21 := L_get_type c15 in
  let a22 := L_get_type c8 in
  let a23 := L_get_type c5 in
  let a24 := L_get_type c2 in
  let a25 := L_get_type c14 in
  let a26 := getLeft a23 in
  let a27 := getLeft a21 in
  let a28 := L_get_type c26 in
  let a29 := getLeft a28 in
  let a30 := L_get_type c17 in
  let a31 := L_get_type c7 in
  let a32 := L_get_type c4 in
  let a33 := havoc t5 t (shift a 0%Z) 1%Z in
  let a34 := L_to_list a33 a 0%Z 1%Z in
  let a35 := getLeft a10 in
  let a36 := havoc t4 a33 (shift a1 0%Z) 1%Z in
  let a37 := L_to_list a36 a1 0%Z 1%Z in
  let a38 := getLeft a12 in
  let a39 := havoc t3 a36 (shift a2 0%Z) 1%Z in
  let a40 := L_to_list a39 a2 0%Z 1%Z in
  let a41 := havoc t2 a39 a4 x5 in
  let a42 := shift a3 (1%Z + x5)%Z in
  let a43 :=
  havoc t1 (map.Map.set (map.Map.set a41 (shift a3 0%Z) i2) a42 i1) a42 1%Z in
  let a44 := getLeft a24 in
  let a45 := L_to_list a43 a4 0%Z 1%Z in
  ~ (rank = 0%Z) -> ~ (x = size) -> ((getLeft a5) = protocol_6) ->
  (a7 = a9) -> ((protocol_1 x2) = a10) -> ((protocol_4 x3) = a8) ->
  ((getLeft a11) = protocol_7) -> ((protocol_2 x2 x3) = a12) ->
  ((split_right a13 0%Z) = a13) -> ((split_right a13 i) = a14) ->
  ((protocol_3 x2 x3 x4) = a15) ->
  ((getLeft a16) = (split_right a13 (1%Z + i)%Z)) -> ((getRight a16) = a9) ->
  (0%Z < size)%Z -> (0%Z < rank)%Z -> (0%Z <= i)%Z -> (rank <= size)%Z ->
  (0%Z <= rank)%Z -> (rank < size)%Z -> (i < x3)%Z -> (0%Z <= x3)%Z ->
  (i <= x3)%Z -> (0%Z <= x5)%Z -> ((-1%Z)%Z <= rank)%Z ->
  ((2%Z + rank)%Z <= size)%Z -> ((-1%Z)%Z <= x5)%Z -> (x5 <= 10000%Z)%Z ->
  size_constrain size -> is_sint32 i -> is_sint32 size -> is_sint32 rank ->
  P_set_type c26 the_protocol -> is_sint32 x1 -> is_sint32 x ->
  is_sint32 x2 -> is_sint32 x3 -> is_sint32 x4 -> is_sint32 x5 ->
  P_set_type c24 (simpl a17) -> P_set_type c22 (simpl a18) ->
  P_set_type c20 (simpl a19) -> P_set_type c18 (assoc (L_get_type c19)) ->
  P_set_type c17 (getRight a20) -> P_set_type c15 (assoc (L_get_type c16)) ->
  P_set_type c14 (getRight a21) -> P_set_type c11 (assoc (L_get_type c12)) ->
  P_set_type c10 (assoc (L_get_type c11)) ->
  P_set_type c8 (assoc (L_get_type c9)) -> P_set_type c7 (getRight a22) ->
  P_set_type c5 (assoc (L_get_type c6)) -> P_set_type c4 (getRight a23) ->
  P_set_type c2 (assoc (L_get_type c3)) -> P_set_type c1 (getRight a24) ->
  P_set_type c (getRight a25) -> isSkip (fsimpl (getLeft a20)) ->
  isSkip (fsimpl (getLeft a25)) -> isSkip (fsimpl (getLeft a22)) ->
  isMessageforIntRecv a26 x 1%Z 0%Z -> isMessageforIntRecv a27 0%Z x5 0%Z ->
  isforIntBroadcast a29 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c16 (Seq (simpl (getLeft a30)) (getRight a30)) ->
  P_set_type c12 (Seq (simpl a14) a7) ->
  P_set_type c6 (Seq (simpl (getLeft a31)) (getRight a31)) ->
  P_set_type c3 (Seq (simpl (getLeft a32)) (getRight a32)) ->
  P_set_type c19 (Seq (split (getLeft a15) rank) (getRight a15)) ->
  P_set_type c9
  (Seq (split protocol_6 (((-1%Z)%Z + size)%Z + ((-1%Z)%Z * rank)%Z)%Z) a11) ->
  predIntBroadcast a29 a34 ->
  isforIntBroadcast a35 0%Z 1%Z (L_to_list a33 a1 0%Z 1%Z) ->
  countiIntBroadcast a29 (getRight a28) a17 a34 ->
  predIntBroadcast a35 a37 ->
  isforIntBroadcast a38 0%Z 1%Z (L_to_list a36 a2 0%Z 1%Z) ->
  countiIntBroadcast a35 (getRight a10) a18 a37 ->
  predIntBroadcast a38 a40 ->
  countiIntBroadcast a38 (getRight a12) a19 a40 ->
  predIntMessage a27 (L_to_list a41 a4 0%Z x5) ->
  predIntMessage a26 (L_to_list a43 a42 0%Z 1%Z) ->
  isPredIntMessage a44 a45 -> isMessageforIntSend a44 x1 1%Z 0%Z a45 ->
...
c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 t6 t7 i1 i2
a5 a6 a7 a8 a9 x2 a10 x3 a11 a12 a13 a14 x4 a15 a16 x5 a17 a18 a19 a20 a21
a22 a23 a24 a25 a26 a27 a28 a29 a30 a31 a32 a33 a34 a35 a36 a37 a38 a39 a40
a41 a42 a43 a44 a45 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16
h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31 h32 h33 h34 h35
h36 h37 h38 h39 h40 h41 h42 h43 h44 h45 h46 h47 h48 h49 h50 h51 h52 h53 h54
h55 h56 h57 h58 h59 h60 h61 h62 h63 h64 h65 h66 h67 h68 h69 h70 h71 h72 h73
h74 h75 h76 h77 h78 h79 h80.
erewrite Q_link by apply h53.
unfold a24.
erewrite Q_link by apply h52.
erewrite Q_link by apply h64.
replace (getLeft a32) with (getLeft (L_get_type c4)) ;[ | unfold a32;reflexivity].
erewrite Q_link by apply h51.
unfold a23.
erewrite Q_link by apply h50.
erewrite Q_link by apply h63.
replace (getLeft a31) with (getLeft (L_get_type c7)) ;[ | unfold a31;reflexivity].
erewrite Q_link by apply h49.
unfold a22.
erewrite Q_link by apply h48.
erewrite Q_link by apply h66.
unfold protocol_6.
rewrite split_foreach by Lia.lia.
rewrite Assoc.
remember (- (1) + size + - (1) * rank)%Z as k.
simpl.
