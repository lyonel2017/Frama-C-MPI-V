...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (c34:S15_c_protocol) (c35:S15_c_protocol)
    (t7:Numbers.BinNums.Z -> Numbers.BinNums.Z)
    (t8:Numbers.BinNums.Z -> Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (i2:Numbers.BinNums.Z),
  let a6 := L_get_type c19 in
  let a7 := L_get_type c9 in
  let a8 := L_get_type c22 in
  let a9 := getRight a8 in
  let a10 := L_get_type c in
  let a11 := getRight a10 in
  let x2 := t6 a in
  let a12 := L_get_type c33 in
  let x3 := t5 a1 in
  let a13 := getRight a6 in
  let a14 := getRight a7 in
  let a15 := L_get_type c31 in
  let a16 := getLeft a10 in
  let a17 := getLeft a8 in
  let a18 := split_right a16 (1%Z + i)%Z in
  let x4 := t4 a2 in
  let a19 := L_get_type c29 in
  let a20 := getRight a13 in
  let x5 := ZArith.BinInt.Z.quot x2 size in
  let a21 := L_get_type c34 in
  let a22 := L_get_type c32 in
  let a23 := L_get_type c30 in
  let a24 := L_get_type c27 in
  let a25 := L_get_type c24 in
  let a26 := L_get_type c17 in
  let a27 := L_get_type c14 in
  let a28 := L_get_type c11 in
  let a29 := L_get_type c10 in
  let a30 := L_get_type c7 in
  let a31 := L_get_type c4 in
  let a32 := L_get_type c23 in
  let a33 := getLeft a27 in
  let a34 := getLeft a31 in
  let a35 := getLeft a25 in
  let a36 := L_get_type c35 in
  let a37 := getLeft a36 in
  let a38 := L_get_type c26 in
  let a39 := L_get_type c16 in
  let a40 := L_get_type c13 in
  let a41 := L_get_type c6 in
  let a42 := L_get_type c3 in
  let a43 := havoc t6 t (shift a 0%Z) 1%Z in
  let a44 := L_to_list a43 a 0%Z 1%Z in
  let a45 := getLeft a12 in
  let a46 := havoc t5 a43 (shift a1 0%Z) 1%Z in
  let a47 := L_to_list a46 a1 0%Z 1%Z in
  let a48 := getLeft a15 in
  let a49 := havoc t4 a46 (shift a2 0%Z) 1%Z in
  let a50 := L_to_list a49 a2 0%Z 1%Z in
  let a51 := havoc t3 a49 a4 x5 in
  let a52 := shift a3 (1%Z + x5)%Z in
  let a53 := havoc t2 (map.Map.set (map.Map.set a51 a5 i2) a52 i1) a52 1%Z in
  let a54 := getLeft a28 in
  let a55 := L_to_list a53 a4 0%Z 1%Z in
  let a56 := havoc t1 a53 a5 1%Z in
  ...
  P_set_type c35 the_protocol -> is_sint32 x1 -> is_sint32 x ->
  is_sint32 x2 -> is_sint32 x3 -> is_sint32 x4 -> is_sint32 x5 ->
  P_set_type c33 (simpl a21) -> P_set_type c31 (simpl a22) ->
  P_set_type c29 (simpl a23) -> P_set_type c27 (assoc (L_get_type c28)) ->
  P_set_type c26 (getRight a24) -> P_set_type c24 (assoc (L_get_type c25)) ->
  P_set_type c23 (getRight a25) -> P_set_type c20 (assoc (L_get_type c21)) ->
  P_set_type c19 (assoc (L_get_type c20)) ->
  P_set_type c17 (assoc (L_get_type c18)) -> P_set_type c16 (getRight a26) ->
  P_set_type c14 (assoc (L_get_type c15)) -> P_set_type c13 (getRight a27) ->
  P_set_type c11 (assoc (L_get_type c12)) -> P_set_type c10 (getRight a28) ->
  P_set_type c9 (getRight a29) -> P_set_type c7 (assoc (L_get_type c8)) ->
  P_set_type c6 (getRight a30) -> P_set_type c4 (assoc (L_get_type c5)) ->
  P_set_type c3 (getRight a31) -> P_set_type c1 (assoc (L_get_type c2)) ->
  P_set_type c (getRight a32) -> isSkip (fsimpl (getLeft a24)) ->
  isSkip (fsimpl (getLeft a32)) -> isSkip (fsimpl (getLeft a26)) ->
  isSkip (fsimpl (getLeft a29)) -> isSkip (fsimpl (getLeft a30)) ->
  isMessageforIntRecv a33 x 1%Z 0%Z -> isMessageforIntRecv a34 x1 1%Z 0%Z ->
  isMessageforIntRecv a35 0%Z x5 0%Z ->
  isforIntBroadcast a37 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c25 (Seq (simpl (getLeft a38)) (getRight a38)) ->
  P_set_type c21 (Seq (simpl a17) a9) ->
  P_set_type c15 (Seq (simpl (getLeft a39)) (getRight a39)) ->
  P_set_type c12 (Seq (simpl (getLeft a40)) (getRight a40)) ->
  P_set_type c8 (Seq (split protocol_7 x1) a14) ->
  P_set_type c5 (Seq (simpl (getLeft a41)) (getRight a41)) ->
  P_set_type c2 (Seq (simpl (getLeft a42)) (getRight a42)) ->
  P_set_type c28 (Seq (split (getLeft a19) rank) (getRight a19)) ->
  P_set_type c18
  (Seq (split protocol_6 (((-1%Z)%Z + size)%Z + ((-1%Z)%Z * rank)%Z)%Z) a13) ->
  predIntBroadcast a37 a44 ->
  isforIntBroadcast a45 0%Z 1%Z (L_to_list a43 a1 0%Z 1%Z) ->
  countiIntBroadcast a37 (getRight a36) a21 a44 ->
  predIntBroadcast a45 a47 ->
  isforIntBroadcast a48 0%Z 1%Z (L_to_list a46 a2 0%Z 1%Z) ->
  countiIntBroadcast a45 (getRight a12) a22 a47 ->
  predIntBroadcast a48 a50 ->
  countiIntBroadcast a48 (getRight a15) a23 a50 ->
  predIntMessage a35 (L_to_list a51 a4 0%Z x5) ->
  predIntMessage a33 (L_to_list a53 a52 0%Z 1%Z) ->
  isPredIntMessage a54 a55 -> isMessageforIntSend a54 x1 1%Z 0%Z a55 ->
  predIntMessage a34 (L_to_list a56 a5 0%Z 1%Z) ->
  ((0%Z < x5)%Z -> (x5 <= 10001%Z)%Z) ->
  (forall (i3:Numbers.BinNums.Z), (0%Z <= i3)%Z -> (i3 <= 10001%Z)%Z ->
   (i3 < 0%Z)%Z \/ (x5 <= i3)%Z -> ((t8 i3) = (t7 i3))) ->
  isMessageforIntSend (getLeft (L_get_type c1)) x 1%Z 0%Z
  (L_to_list a56 (shift a3 x5) 0%Z 1%Z).
Proof.
...
c28 c29 c30 c31 c32 c33 c34 c35 t7 t8 i1 i2 a6 a7 a8 a9 a10 a11 x2 a12 x3 a13
a14 a15 a16 a17 a18 x4 a19 a20 x5 a21 a22 a23 a24 a25 a26 a27 a28 a29 a30 a31
a32 a33 a34 a35 a36 a37 a38 a39 a40 a41 a42 a43 a44 a45 a46 a47 a48 a49 a50
a51 a52 a53 a54 a55 a56 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15
h16 h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31 h32 h33 h34
h35 h36 h37 h38 h39 h40 h41 h42 h43 h44 h45 h46 h47 h48 h49 h50 h51 h52 h53
h54 h55 h56 h57 h58 h59 h60 h61 h62 h63 h64 h65 h66 h67 h68 h69 h70 h71 h72
h73 h74 h75 h76 h77 h78 h79 h80 h81 h82 h83 h84 h85 h86 h87 h88 h89 h90 h91
h92 h93 h94 h95 h96.
erewrite Q_link by apply h62.
erewrite Q_link by apply h79.
replace (getLeft a42) with (getLeft (L_get_type c3)) ;[ | unfold a42;reflexivity].
erewrite Q_link by apply h61.
unfold a31.
erewrite Q_link by apply h60.
erewrite Q_link by apply h78.
replace (getLeft a41) with (getLeft (L_get_type c6)) ;[ | unfold a41;reflexivity].
erewrite Q_link by apply h59.
unfold a30.
erewrite Q_link by apply h58.
erewrite Q_link by apply h77.
unfold protocol_7.
