...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (t6:Numbers.BinNums.Z -> Numbers.BinNums.Z)
    (t7:Numbers.BinNums.Z -> Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (i2:Numbers.BinNums.Z),
  let a5 := L_get_type c9 in
  let a6 := L_get_type c12 in
  let a7 := getRight a6 in
  let a8 := L_get_type c in
  let a9 := getRight a8 in
  let x2 := t5 a in
  let a10 := L_get_type c23 in
  let x3 := t4 a1 in
  let a11 := getRight a5 in
  let a12 := L_get_type c21 in
  let a13 := getLeft a8 in
  let a14 := getLeft a6 in
  let x4 := t3 a2 in
  let a15 := L_get_type c19 in
  let a16 := getRight a11 in
  let x5 := ZArith.BinInt.Z.quot x2 size in
  let a17 := L_get_type c24 in
  let a18 := L_get_type c22 in
  let a19 := L_get_type c20 in
  let a20 := L_get_type c17 in
  let a21 := L_get_type c14 in
  let a22 := L_get_type c7 in
  let a23 := L_get_type c4 in
  let a24 := L_get_type c13 in
  let a25 := getLeft a23 in
  let a26 := getLeft a21 in
  let a27 := L_get_type c25 in
  let a28 := getLeft a27 in
  let a29 := L_get_type c16 in
  let a30 := L_get_type c6 in
  let a31 := L_get_type c3 in
  let a32 := havoc t5 t (shift a 0%Z) 1%Z in
  let a33 := L_to_list a32 a 0%Z 1%Z in
  let a34 := getLeft a10 in
  let a35 := havoc t4 a32 (shift a1 0%Z) 1%Z in
  let a36 := L_to_list a35 a1 0%Z 1%Z in
  let a37 := getLeft a12 in
  let a38 := havoc t3 a35 (shift a2 0%Z) 1%Z in
  let a39 := L_to_list a38 a2 0%Z 1%Z in
  let a40 := havoc t2 a38 a4 x5 in
  let a41 := shift a3 (1%Z + x5)%Z in
  let a42 :=
  havoc t1 (map.Map.set (map.Map.set a40 (shift a3 0%Z) i2) a41 i1) a41 1%Z in
  ...
  (0%Z < size)%Z -> (0%Z <= i)%Z -> (0%Z <= rank)%Z -> (rank < size)%Z ->
  (i < x3)%Z -> (0%Z <= x3)%Z -> (i <= x3)%Z -> (0%Z <= x5)%Z ->
  ((-1%Z)%Z <= rank)%Z -> ((2%Z + rank)%Z <= size)%Z -> ((-1%Z)%Z <= x5)%Z ->
  (x5 <= 10000%Z)%Z -> size_constrain size -> is_sint32 i ->
  is_sint32 size -> is_sint32 rank -> P_set_type c25 the_protocol ->
  is_sint32 x1 -> is_sint32 x -> is_sint32 x2 -> is_sint32 x3 ->
  is_sint32 x4 -> is_sint32 x5 -> P_set_type c23 (simpl a17) ->
  P_set_type c21 (simpl a18) -> P_set_type c19 (simpl a19) ->
  P_set_type c17 (assoc (L_get_type c18)) -> P_set_type c16 (getRight a20) ->
  P_set_type c14 (assoc (L_get_type c15)) -> P_set_type c13 (getRight a21) ->
  P_set_type c10 (assoc (L_get_type c11)) ->
  P_set_type c9 (assoc (L_get_type c10)) ->
  P_set_type c7 (assoc (L_get_type c8)) -> P_set_type c6 (getRight a22) ->
  P_set_type c4 (assoc (L_get_type c5)) -> P_set_type c3 (getRight a23) ->
  P_set_type c1 (assoc (L_get_type c2)) -> P_set_type c (getRight a24) ->
  isSkip (fsimpl (getLeft a20)) -> isSkip (fsimpl (getLeft a24)) ->
  isSkip (fsimpl (getLeft a22)) -> isMessageforIntRecv a25 x 1%Z 0%Z ->
  isMessageforIntRecv a26 0%Z x5 0%Z ->
  isforIntBroadcast a28 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c15 (Seq (simpl (getLeft a29)) (getRight a29)) ->
  P_set_type c11 (Seq (simpl a14) a7) ->
  P_set_type c5 (Seq (simpl (getLeft a30)) (getRight a30)) ->
  P_set_type c2 (Seq (simpl (getLeft a31)) (getRight a31)) ->
  P_set_type c18 (Seq (split (getLeft a15) rank) (getRight a15)) ->
  P_set_type c8
  (Seq (split protocol_6 (((-1%Z)%Z + size)%Z + ((-1%Z)%Z * rank)%Z)%Z) a11) ->
  predIntBroadcast a28 a33 ->
  isforIntBroadcast a34 0%Z 1%Z (L_to_list a32 a1 0%Z 1%Z) ->
  countiIntBroadcast a28 (getRight a27) a17 a33 ->
  predIntBroadcast a34 a36 ->
  isforIntBroadcast a37 0%Z 1%Z (L_to_list a35 a2 0%Z 1%Z) ->
  countiIntBroadcast a34 (getRight a10) a18 a36 ->
  predIntBroadcast a37 a39 ->
  countiIntBroadcast a37 (getRight a12) a19 a39 ->
  predIntMessage a26 (L_to_list a40 a4 0%Z x5) ->
  predIntMessage a25 (L_to_list a42 a41 0%Z 1%Z) ->
  ((0%Z < x5)%Z -> (x5 <= 10001%Z)%Z) ->
  (forall (i3:Numbers.BinNums.Z), (0%Z <= i3)%Z -> (i3 <= 10001%Z)%Z ->
   (i3 < 0%Z)%Z \/ (x5 <= i3)%Z -> ((t7 i3) = (t6 i3))) ->
  isMessageforIntSend (getLeft (L_get_type c1)) x1 1%Z 0%Z
  (L_to_list a42 a4 0%Z 1%Z).
Proof.
intros x a a1 a2 x1 a3 a4 t t1 t2 t3 t4 t5 i c c1 c2 c3 c4 c5 c6 c7 c8 c9 c10
c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 t6 t7 i1 i2 a5 a6
a7 a8 a9 x2 a10 x3 a11 a12 a13 a14 x4 a15 a16 x5 a17 a18 a19 a20 a21 a22 a23
...
h60 h61 h62 h63 h64 h65 h66 h67 h68 h69 h70 h71 h72 h73 h74 h75.
erewrite Q_link by apply h50.
erewrite Q_link by apply h61.
replace (getLeft a31) with (getLeft (L_get_type c3)) ;[ | unfold a31;reflexivity].
erewrite Q_link by apply h49.
unfold a23.
erewrite Q_link by apply h48.
erewrite Q_link by apply h60.
replace (getLeft a30) with (getLeft (L_get_type c6)) ;[ | unfold a30;reflexivity].
erewrite Q_link by apply h47.
unfold a22.
erewrite Q_link by apply h46.
