...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (t6:addr -> Numbers.BinNums.Z) (i:Numbers.BinNums.Z) (c:S15_c_protocol)
    ...
    (c13:S15_c_protocol) (c14:S15_c_protocol) (c15:S15_c_protocol)
    (c16:S15_c_protocol) (c17:S15_c_protocol) (c18:S15_c_protocol)
    (c19:S15_c_protocol) (c20:S15_c_protocol) (c21:S15_c_protocol)
    (c22:S15_c_protocol) (c23:S15_c_protocol) (c24:S15_c_protocol)
    (c25:S15_c_protocol) (c26:S15_c_protocol) (c27:S15_c_protocol)
    (c28:S15_c_protocol) (c29:S15_c_protocol) (c30:S15_c_protocol)
    (c31:S15_c_protocol) (c32:S15_c_protocol) (c33:S15_c_protocol)
    (c34:S15_c_protocol) (c35:S15_c_protocol) (c36:S15_c_protocol)
    (t7:Numbers.BinNums.Z -> Numbers.BinNums.Z)
    (t8:Numbers.BinNums.Z -> Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (i2:Numbers.BinNums.Z),
  let a6 := L_get_type c20 in
  let a7 := L_get_type c10 in
  let a8 := L_get_type c23 in
  let a9 := getRight a8 in
  let a10 := L_get_type c in
  let a11 := getRight a10 in
  let x2 := t6 a in
  let a12 := L_get_type c34 in
  let x3 := t5 a1 in
  let a13 := getRight a6 in
  let a14 := getRight a7 in
  let a15 := L_get_type c32 in
  let a16 := getLeft a10 in
  let a17 := getLeft a8 in
  let a18 := split_right a16 (1%Z + i)%Z in
  let x4 := t4 a2 in
  let a19 := L_get_type c30 in
  let a20 := getRight a13 in
  let x5 := ZArith.BinInt.Z.quot x2 size in
  let a21 := L_get_type c35 in
  let a22 := L_get_type c33 in
  let a23 := L_get_type c31 in
  let a24 := L_get_type c28 in
  let a25 := L_get_type c25 in
  let a26 := L_get_type c18 in
  let a27 := L_get_type c15 in
  let a28 := L_get_type c12 in
  let a29 := L_get_type c11 in
  let a30 := L_get_type c8 in
  let a31 := L_get_type c5 in
  let a32 := L_get_type c2 in
  let a33 := L_get_type c24 in
  let a34 := getLeft a27 in
  let a35 := getLeft a31 in
  let a36 := getLeft a25 in
  let a37 := L_get_type c36 in
  let a38 := getLeft a37 in
  let a39 := L_get_type c27 in
  let a40 := L_get_type c17 in
  let a41 := L_get_type c14 in
  let a42 := L_get_type c7 in
  let a43 := L_get_type c4 in
  let a44 := havoc t6 t (shift a 0%Z) 1%Z in
  let a45 := L_to_list a44 a 0%Z 1%Z in
  let a46 := getLeft a12 in
  let a47 := havoc t5 a44 (shift a1 0%Z) 1%Z in
  let a48 := L_to_list a47 a1 0%Z 1%Z in
  let a49 := getLeft a15 in
  let a50 := havoc t4 a47 (shift a2 0%Z) 1%Z in
  let a51 := L_to_list a50 a2 0%Z 1%Z in
  let a52 := havoc t3 a50 a4 x5 in
  let a53 := shift a3 (1%Z + x5)%Z in
  let a54 := havoc t2 (map.Map.set (map.Map.set a52 a5 i2) a53 i1) a53 1%Z in
  let a55 := getLeft a28 in
  let a56 := L_to_list a54 a4 0%Z 1%Z in
  let a57 := havoc t1 a54 a5 1%Z in
  let a58 := getLeft a32 in
  let a59 := L_to_list a57 (shift a3 x5) 0%Z 1%Z in
  ~ (rank = 0%Z) -> ~ (x = size) -> ((getLeft a6) = protocol_6) ->
  ((getLeft a7) = protocol_7) -> (a9 = a11) -> ((protocol_1 x2) = a12) ->
  ((protocol_4 x3) = a10) -> ((getLeft a13) = protocol_7) ->
  ((getRight a14) = a11) -> ((protocol_2 x2 x3) = a15) ->
  ((split_right a16 0%Z) = a16) -> ((split_right a16 i) = a17) ->
  (a18 = (getLeft a14)) -> ((protocol_3 x2 x3 x4) = a19) ->
  ((getLeft a20) = a18) -> ((getRight a20) = a11) -> (0%Z < size)%Z ->
  (0%Z < rank)%Z -> (0%Z <= i)%Z -> (rank <= size)%Z -> (0%Z <= rank)%Z ->
  (rank < size)%Z -> (i < x3)%Z -> (0%Z <= x3)%Z -> (i <= x3)%Z ->
  (0%Z <= x5)%Z -> ((2%Z + rank)%Z <= size)%Z -> ((-1%Z)%Z <= x5)%Z ->
  (x5 <= 10000%Z)%Z -> (x5 <= 10001%Z)%Z -> size_constrain size ->
  is_sint32 i -> is_sint32 size -> is_sint32 rank ->
  P_set_type c36 the_protocol -> is_sint32 x1 -> is_sint32 x ->
  is_sint32 x2 -> is_sint32 x3 -> is_sint32 x4 -> is_sint32 x5 ->
  P_set_type c34 (simpl a21) -> P_set_type c32 (simpl a22) ->
  P_set_type c30 (simpl a23) -> P_set_type c28 (assoc (L_get_type c29)) ->
  P_set_type c27 (getRight a24) -> P_set_type c25 (assoc (L_get_type c26)) ->
  P_set_type c24 (getRight a25) -> P_set_type c21 (assoc (L_get_type c22)) ->
  P_set_type c20 (assoc (L_get_type c21)) ->
  P_set_type c18 (assoc (L_get_type c19)) -> P_set_type c17 (getRight a26) ->
  P_set_type c15 (assoc (L_get_type c16)) -> P_set_type c14 (getRight a27) ->
  P_set_type c12 (assoc (L_get_type c13)) -> P_set_type c11 (getRight a28) ->
  P_set_type c10 (getRight a29) -> P_set_type c8 (assoc (L_get_type c9)) ->
  P_set_type c7 (getRight a30) -> P_set_type c5 (assoc (L_get_type c6)) ->
  P_set_type c4 (getRight a31) -> P_set_type c2 (assoc (L_get_type c3)) ->
  P_set_type c1 (getRight a32) -> P_set_type c (getRight a33) ->
  isSkip (fsimpl (getLeft a24)) -> isSkip (fsimpl (getLeft a33)) ->
  isSkip (fsimpl (getLeft a26)) -> isSkip (fsimpl (getLeft a29)) ->
  isSkip (fsimpl (getLeft a30)) -> isMessageforIntRecv a34 x 1%Z 0%Z ->
  isMessageforIntRecv a35 x1 1%Z 0%Z -> isMessageforIntRecv a36 0%Z x5 0%Z ->
  isforIntBroadcast a38 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c26 (Seq (simpl (getLeft a39)) (getRight a39)) ->
  P_set_type c22 (Seq (simpl a17) a9) ->
  P_set_type c16 (Seq (simpl (getLeft a40)) (getRight a40)) ->
  P_set_type c13 (Seq (simpl (getLeft a41)) (getRight a41)) ->
  P_set_type c9 (Seq (split protocol_7 x1) a14) ->
  P_set_type c6 (Seq (simpl (getLeft a42)) (getRight a42)) ->
  P_set_type c3 (Seq (simpl (getLeft a43)) (getRight a43)) ->
  P_set_type c29 (Seq (split (getLeft a19) rank) (getRight a19)) ->
  P_set_type c19
  (Seq (split protocol_6 (((-1%Z)%Z + size)%Z + ((-1%Z)%Z * rank)%Z)%Z) a13) ->
  predIntBroadcast a38 a45 ->
  isforIntBroadcast a46 0%Z 1%Z (L_to_list a44 a1 0%Z 1%Z) ->
  countiIntBroadcast a38 (getRight a37) a21 a45 ->
  predIntBroadcast a46 a48 ->
  isforIntBroadcast a49 0%Z 1%Z (L_to_list a47 a2 0%Z 1%Z) ->
  countiIntBroadcast a46 (getRight a12) a22 a48 ->
  predIntBroadcast a49 a51 ->
  countiIntBroadcast a49 (getRight a15) a23 a51 ->
  predIntMessage a36 (L_to_list a52 a4 0%Z x5) ->
  predIntMessage a34 (L_to_list a54 a53 0%Z 1%Z) ->
  isPredIntMessage a55 a56 -> isMessageforIntSend a55 x1 1%Z 0%Z a56 ->
  predIntMessage a35 (L_to_list a57 a5 0%Z 1%Z) ->
  isPredIntMessage a58 a59 -> isMessageforIntSend a58 x 1%Z 0%Z a59 ->
  (forall (i3:Numbers.BinNums.Z), (0%Z <= i3)%Z -> (i3 <= 10001%Z)%Z ->
   (i3 < 0%Z)%Z \/ (x5 <= i3)%Z -> ((t8 i3) = (t7 i3))) ->
  isSkip (fsimpl (getLeft (L_get_type c1))).
Proof.
intros x a a1 a2 x1 a3 a4 a5 t t1 t2 t3 t4 t5 t6 i c c1 c2 c3 c4 c5 c6 c7 c8
c9 c10 c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 c27
c28 c29 c30 c31 c32 c33 c34 c35 c36 t7 t8 i1 i2 a6 a7 a8 a9 a10 a11 x2 a12 x3
a13 a14 a15 a16 a17 a18 x4 a19 a20 x5 a21 a22 a23 a24 a25 a26 a27 a28 a29 a30
a31 a32 a33 a34 a35 a36 a37 a38 a39 a40 a41 a42 a43 a44 a45 a46 a47 a48 a49
a50 a51 a52 a53 a54 a55 a56 a57 a58 a59 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11
h12 h13 h14 h15 h16 h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28 h29 h30
h31 h32 h33 h34 h35 h36 h37 h38 h39 h40 h41 h42 h43 h44 h45 h46 h47 h48 h49
h50 h51 h52 h53 h54 h55 h56 h57 h58 h59 h60 h61 h62 h63 h64 h65 h66 h67 h68
h69 h70 h71 h72 h73 h74 h75 h76 h77 h78 h79 h80 h81 h82 h83 h84 h85 h86 h87
h88 h89 h90 h91 h92 h93 h94 h95 h96 h97 h98.
erewrite Q_link by apply h63.
unfold a32.
erewrite Q_link by apply h62.
erewrite Q_link by apply h80.
replace (getLeft a43) with (getLeft (L_get_type c4)) ;[ | unfold a43;reflexivity].
erewrite Q_link by apply h61.
unfold a31.
erewrite Q_link by apply h60.
erewrite Q_link by apply h79.
replace (getLeft a42) with (getLeft (L_get_type c7)) ;[ | unfold a42;reflexivity].
erewrite Q_link by apply h59.
unfold a30.
erewrite Q_link by apply h58.
erewrite Q_link by apply h78.
unfold protocol_7.
