...
Theorem wp_goal :
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (c11:S15_c_protocol) (c12:S15_c_protocol),
  let x := t4 a in
  let a4 := L_get_type c10 in
  let x1 := t3 a1 in
  let a5 := L_get_type c8 in
  let x2 := t2 a2 in
  let a6 := L_get_type c6 in
  let x3 := ZArith.BinInt.Z.quot x size in
  let a7 := L_get_type c11 in
  let a8 := L_get_type c9 in
  let a9 := L_get_type c7 in
  let a10 := L_get_type c4 in
  let a11 := L_get_type c1 in
  let a12 := getLeft a11 in
  let a13 := L_get_type c12 in
  let a14 := getLeft a13 in
  let a15 := L_get_type c3 in
  let a16 := havoc t4 t (shift a 0%Z) 1%Z in
  let a17 := L_to_list a16 a 0%Z 1%Z in
  let a18 := getLeft a4 in
  let a19 := havoc t3 a16 (shift a1 0%Z) 1%Z in
  let a20 := L_to_list a19 a1 0%Z 1%Z in
  let a21 := getLeft a5 in
  let a22 := havoc t2 a19 (shift a2 0%Z) 1%Z in
  let a23 := L_to_list a22 a2 0%Z 1%Z in
  ~ (rank = 0%Z) -> ((protocol_1 x) = a4) -> ((protocol_2 x x1) = a5) ->
  ((protocol_3 x x1 x2) = a6) -> (0%Z < size)%Z -> (0%Z <= rank)%Z ->
  (rank < size)%Z -> (0%Z <= x3)%Z -> size_constrain size ->
  is_sint32 size -> is_sint32 rank -> P_set_type c12 the_protocol ->
  is_sint32 x -> is_sint32 x1 -> is_sint32 x2 -> is_sint32 x3 ->
  P_set_type c10 (simpl a7) -> P_set_type c8 (simpl a8) ->
  P_set_type c6 (simpl a9) -> P_set_type c4 (assoc (L_get_type c5)) ->
  P_set_type c3 (getRight a10) -> P_set_type c1 (assoc (L_get_type c2)) ->
  P_set_type c (getRight a11) -> isSkip (fsimpl (getLeft a10)) ->
  isMessageforIntRecv a12 0%Z x3 0%Z ->
  isforIntBroadcast a14 0%Z 1%Z (L_to_list t a 0%Z 1%Z) ->
  P_set_type c2 (Seq (simpl (getLeft a15)) (getRight a15)) ->
  P_set_type c5 (Seq (split (getLeft a6) rank) (getRight a6)) ->
  predIntBroadcast a14 a17 ->
  isforIntBroadcast a18 0%Z 1%Z (L_to_list a16 a1 0%Z 1%Z) ->
  countiIntBroadcast a14 (getRight a13) a7 a17 -> predIntBroadcast a18 a20 ->
  isforIntBroadcast a21 0%Z 1%Z (L_to_list a19 a2 0%Z 1%Z) ->
  countiIntBroadcast a18 (getRight a4) a8 a20 -> predIntBroadcast a21 a23 ->
  countiIntBroadcast a21 (getRight a5) a9 a23 ->
  predIntMessage a12 (L_to_list (havoc t1 a22 a3 x3) a3 0%Z x3) ->
  ((0%Z < x3)%Z -> (x3 <= 10001%Z)%Z) ->
  isSkip (fsimpl (getLeft (L_get_type c))).
(* Why3 intros a a1 a2 a3 t t1 t2 t3 t4 c c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11
        c12 x a4 x1 a5 x2 a6 x3 a7 a8 a9 a10 a11 a12 a13 a14 a15 a16 a17 a18
        a19 a20 a21 a22 a23 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14
        h15 h16 h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31
        h32 h33 h34 h35 h36 h37 h38. *)
Proof.
intros a a1 a2 a3 t t1 t2 t3 t4 c c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 x a4
x1 a5 x2 a6 x3 a7 a8 a9 a10 a11 a12 a13 a14 a15 a16 a17 a18 a19 a20 a21 a22
a23 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16 h17 h18 h19 h20
h21 h22 h23 h24 h25 h26 h27 h28 h29 h30 h31 h32 h33 h34 h35 h36 h37 h38.
erewrite Q_link by apply h23.
unfold a11.
erewrite Q_link by apply h22.
erewrite Q_link by apply h27.
replace (getLeft a15) with (getLeft (L_get_type c3)) ;[ | unfold a15;reflexivity].
erewrite Q_link by apply h21.
unfold a10.
erewrite Q_link by apply h20.
erewrite Q_link by apply h28.
rewrite <- h4.
unfold protocol_3.
simpl.
