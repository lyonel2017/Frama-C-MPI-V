...
int main(int argc, char** argv) {
  ...
  /*@ requires get_type(protocol) == protocol_3;
    ...
    @ ensures get_type(protocol) == protocol_6;
    @*/
  if (rank == 0) {
  ...
  /*@ requires get_type(protocol) == protocol_6;
    @ assigns protocol, u[0], u[local_n+1], iter,u_new[0..local_n-1];
    @ ensures isSkip(get_type(protocol));
    @*/
  /*@ loop invariant 0 <= iter <= NUMITER;
    @ loop invariant iter < NUMITER ==> getFirst(get_type(protocol)) ==
    @   getNext(split(getFirst(\at(get_type(protocol),l11)),iter));
    @ loop invariant iter < NUMITER ==> getNext(get_type(protocol)) ==
    @   getNext(\at(get_type(protocol),l11));
    @ loop invariant iter == NUMITER ==> get_type(protocol) ==
    @   getNext(\at(get_type(protocol),l11));
    @ loop assigns u[0], u[local_n+1], protocol, iter, u_new[0..local_n-1];
  ...
  for (iter = 0; iter < NUMITER; iter++) {
    ...
    /*@ requires getFirst(get_type(protocol)) == protocol_4;
      ...
      @   getNext(split(getFirst(\at(get_type(protocol),l11)),iter+1));
