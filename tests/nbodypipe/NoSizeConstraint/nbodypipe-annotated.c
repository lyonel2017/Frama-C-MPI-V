...
int main(int argc,char** argv) {
  ...
  // shortened by 1 s.t. it start at 0
  //@ ghost l01:;
  /*@ requires getFirst(get_type(protocol)) == the_protocol;
    @ assigns protocol, iter, pipe, recvbuf[0..npart*4-1];
  ...
  /*@ loop invariant 0 <= iter <= NUM_ITER;
    ...
    @   getNext(split(getFirst(\at(get_type(protocol),l01)),iter));
  ...
  for (iter = 0; iter < NUM_ITER; iter++) {
    ...
    /*@ loop invariant 0 <= pipe <= procs - 1;
      ...
      @ loop invariant procs == 1 ==> get_type(protocol) ==
      @   \at(get_type(protocol),l0);
      @ loop invariant getNext(get_type(protocol)) ==
    ...
    for (pipe=0; pipe < procs - 1; pipe++) {
      /*@ ghost
        ...
        assoc();
        @*/
      /** ISSUE:ENSURES getNext(..) == getNext(..) CANNOT BE VERIFIED **/
      /*@ requires procs > 1;
      ...
      if (rank == 0) {
        ...
        /*@ ghost
          ...
          @ loop invariant rank > 1 ==> getFirst(get_type(protocol)) ==
          @  getNext(split(getFirst(\at(get_type(protocol),l2)),j));
          @ loop invariant rank == 1 ==> get_type(protocol) ==
          @   \at(get_type(protocol), l2);
          @ loop invariant getNext(get_type(protocol)) ==
          ...
          /*@ requires procs > 1;
            ...
            @   getNext(\at(get_type(protocol),l2));
            @*/
          /** ISSUE: isProtocolForSend cannot be verified **/
          MPI_Ssend(sendbuf, npart * 4, MPI_FLOAT, right, 0, MPI_COMM_WORLD);
          /*@ ghost
            j = rank + 1;
