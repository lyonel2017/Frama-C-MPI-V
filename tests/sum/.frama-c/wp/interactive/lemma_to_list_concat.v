...
Theorem wp_goal :
  ...
  (i1 <= i2)%Z -> (i <= i1)%Z ->
  vlist_eq (L_to_list t a i i2)
  (concat (L_to_list t a i i1) (L_to_list t a i1 i2)).
(* Why3 intros t a i i1 i2 h1 h2. *)
Proof.
intros.
apply Zlt_lower_bound_ind with
(P:= fun k => 
    vlist_eq (L_to_list t a i k) (concat (L_to_list t a i i1) (L_to_list t a i1 k)))
    (z := i1) ;[| Lia.lia].
intros.
assert (H3:(i1 = x \/ i1 < x)%Z) by Lia.lia.
destruct H3.
- rewrite H3.
  rewrite (Q_to_list_empty t a x x) by Lia.lia.
  rewrite rw_nil_concat_right.
  rewrite <- rw_repeat_one at 1.
  apply eq_repeat_one.
- replace x with (1 + (x-1))%Z by Lia.lia.
  rewrite (Q_to_list_cons t a i (1 + (x -1))) by Lia.lia.
  rewrite (Q_to_list_cons t a i1 (1 + (x -1))) by Lia.lia.
  replace (- (1) + (1 + (x - 1)))%Z with (x-1)%Z by Lia.lia.
  assert(H4 : (i1 <= x - 1 < x)%Z) by Lia.lia.
  specialize (H1 (x-1)%Z H4).
  apply extensionality in H1.
  rewrite H1.
 apply eq_assoc_concat.
Qed.