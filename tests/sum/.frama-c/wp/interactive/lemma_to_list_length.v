...
Theorem wp_goal :
  ...
  (i <= i1)%Z -> ((i + (length (L_to_list t a i i1)))%Z = i1).
(* Why3 intros t a i i1 h1. *)
Proof.
intros t a i i1 h1.
apply Zlt_lower_bound_ind with
  (P:= fun k => (i + length (L_to_list t a i k))%Z = k )
  ( z:= i).
intros.
assert ( H2: (i = x \/ i < x)%Z) by Lia.lia.
destruct H2.
* subst.
  ...
  rewrite Z.add_0_r.
  reflexivity.
* rewrite Q_to_list_cons by assumption.
