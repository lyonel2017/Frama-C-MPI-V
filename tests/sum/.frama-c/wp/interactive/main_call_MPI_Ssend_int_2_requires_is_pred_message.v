...
Axiom Q_sum_4 :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (t1:addr) (t2:addr) (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z),
  (forall (i1:Numbers.BinNums.Z), (i <= i1)%Z -> (i1 < j)%Z ->
   ((Mint1 (shift t1 i1)) = (Mint (shift t2 i1)))) ->
  ((L_sum Mint1 t1 i j) = (L_sum Mint t2 i j)).
...
(* Why3 goal *)
Theorem wp_goal :
  let a := global 1934%Z in
  let a1 := global 1936%Z in
  let a2 := shift a1 0%Z in
  let a3 := global 1921%Z in
  ...
  forall (t:addr -> Numbers.BinNums.Z) (t1:addr -> Numbers.BinNums.Z)
    ...
    (c10:S15_c_protocol) (c11:S15_c_protocol) (i6:Numbers.BinNums.Z),
  let a5 := L_get_type c4 in
  let a6 := getNext a5 in
  let a7 := L_get_type c1 in
  let a8 := L_get_type c in
  let a9 := getNext a8 in
  let a10 := L_get_type c9 in
  let a11 := getNext a10 in
  let a12 := L_get_type c5 in
  let a13 := getNext a12 in
  let a14 := getFirst a5 in
  let a15 := getFirst a8 in
  let a16 := getFirst a10 in
  let a17 := getFirst a12 in
  let a18 := havoc t2 t a2 1000%Z in
  let a19 := L_to_list a18 a2 0%Z 1000%Z in
  let a20 := havoc t1 a18 a4 i5 in
  let a21 := map.Map.set a20 a i6 in
  let x := (i5 * rank)%Z in
  let a22 := L_get_type c7 in
  let a23 := L_get_type c10 in
  let a24 := getFirst a22 in
  let a25 := L_get_type c11 in
  let a26 := getFirst a25 in
  ~ (rank = 0%Z) -> ((t a) = 0%Z) -> (a6 = (getNext a7)) -> (a9 = a7) ->
  (a11 = a9) -> (a13 = a9) -> ((getNext (split (getFirst a7) i3)) = a14) ->
  ((getNext (split a15 i4)) = a16) -> ((getNext (split a15 i2)) = a17) ->
  ((f a19) = a8) -> ((L_sum a21 a4 0%Z i1) = i6) ->
  ((L_sum a21 a2 x (i1 + x)%Z) = i6) -> (0%Z < i4)%Z -> (0%Z < i3)%Z ->
  (0%Z < size)%Z -> (0%Z <= i5)%Z -> (i1 <= i5)%Z -> (rank <= i4)%Z ->
  (rank <= i3)%Z -> (i <= i2)%Z -> (0%Z <= i1)%Z -> (i5 <= i1)%Z ->
  (i2 <= i)%Z -> (0%Z <= rank)%Z -> (i4 <= rank)%Z -> (i3 <= rank)%Z ->
  (rank < i2)%Z -> (rank < i)%Z -> (rank < size)%Z ->
  ((i1 * (1%Z + i3)%Z)%Z <= 1000%Z)%Z -> size_constrain size ->
  is_sint32 i6 -> is_sint32 i5 -> is_sint32 i4 -> is_sint32 i3 ->
  is_sint32 i2 -> is_sint32 i1 -> is_sint32 i -> is_sint32 size ->
  is_sint32 rank -> P_set_type c11 the_protocol -> is_sint32 (1%Z + i4)%Z ->
  P_set_type c7 (assoc (L_get_type c8)) -> P_set_type c6 (getNext a22) ->
  P_set_type c2 (assoc (L_get_type c3)) -> P_set_type c1 a13 ->
  P_set_type c (simpl a23) -> isSkip (simpl a17) ->
  isMessageforIntRecv a24 0%Z i5 1%Z ->
  P_set_type c8 (Seq (simpl a16) a11) ->
  P_set_type c3 (Seq (simpl a14) a6) ->
  isforIntGhostBroadcast a26 0%Z 1000%Z (L_to_list t a2 0%Z 1000%Z) ->
  predIntBroadcast a26 a19 -> countiIntBroadcast a26 (getNext a25) a23 a19 ->
  predIntMessage a24 (L_to_list a20 a4 0%Z i5) ->
  (size <= 1000%Z)%Z /\ (size = i) /\ ((ZArith.BinInt.Z.quot 1000%Z i) = i5) \/
  ~ (size <= 1000%Z)%Z /\ (i5 = 1%Z) /\ (i = 1000%Z) ->
  ((0%Z < i5)%Z -> (i5 <= 1000%Z)%Z) ->
  (forall (i7:Numbers.BinNums.Z), ~ (i4 = i) -> (0%Z <= i7)%Z ->
   (i7 < i5)%Z -> ((i7 + (i4 * i5)%Z)%Z <= 999%Z)%Z) ->
  (forall (i7:Numbers.BinNums.Z), (0%Z <= i7)%Z -> (i7 < i5)%Z ->
   ((a20 (shift a3 i7)) = (a18 (shift a1 (i7 + x)%Z)))) ->
  isPredIntMessage (getFirst (L_get_type c2))
  (L_to_list (map.Map.set (havoc t1 a18 a4 i1) a i6) a 0%Z 1%Z).
(* Why3 intros a a1 a2 a3 a4 t t1 t2 i i1 i2 i3 i4 i5 c c1 c2 c3 c4 c5 c6 c7
        c8 c9 c10 c11 i6 a5 a6 a7 a8 a9 a10 a11 a12 a13 a14 a15 a16 a17 a18
        a19 a20 a21 x a22 a23 a24 a25 a26 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11
        h12 h13 h14 h15 h16 h17 h18 h19 h20 h21 h22 h23 h24 h25 h26 h27 h28
        h29 h30 h31 h32 h33 h34 h35 h36 h37 h38 h39 h40 h41 h42 h43 h44 h45
        h46 h47 h48 h49 h50 h51 h52 h53 h54 h55 h56 h57 h58 h59. *)
Proof.
intros a a1 a2 a3 a4 t t1 t2 i i1 i2 i3 i4 i5 c c1 c2 c3 c4 c5 c6 c7 c8 c9
c10 c11 i6 a5 a6 a7 a8 a9 a10 a11 a12 a13 a14 a15 a16 a17 a18 a19 a20 a21 a22
a23 a24 a25 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16 h17 h18
...
h57 h58 h59 h60 h61.
unfold a8 in h12.
erewrite Q_link in h12 by apply h49.
rewrite <- h12 in h49.
erewrite Q_link by apply h47.
...
unfold f.
simpl.
assert(H: i3 = rank) by Lia.lia.
assert(H1: i4 = rank) by Lia.lia.
rewrite split_foreach1.
rewrite test1.
rewrite simpl_foreach_1.
rewrite Assoc.
simpl.
apply isPredIntMessage1.
apply p2_closure_def.
unfold p2.
rewrite <- Q_to_list_nth.
simpl.
...
specialize (Map.set'def (havoc t1 a18 a4 i1) a i6 a).
...
assert (H4 : forall i j,  (i <= j)%Z -> (0 <= i)%Z  -> (i <= j <= 1000)%Z -> 
 sum (fun y1 : int => nth (L_to_list a18 a2 0 1000) y1) i j = L_sum (Map.set a20 a i6) a2 i j).
{ intros i0 j H15.
  apply Zlt_lower_bound_ind with
  (P:= fun k =>(0 <= i0)%Z -> (i0 <= k <= 1000)%Z -> 
   sum (fun y1 : int => nth (L_to_list a18 a2 0 1000) y1) i0 k = L_sum (Map.set a20 a i6) a2 i0 k)
  ( z:= i0).
  intros.
  assert ( H7: (i0 = x \/ i0 < x)%Z) by Lia.lia.
  destruct H7.
  * rewrite H7.
    specialize (sum'def (fun y1 : int => nth (L_to_list a18 a2 0 1000) y1) x x).
    intros H11. destruct H11 as (H11 & _).
    rewrite H11 by Lia.lia.
    rewrite Q_sum1 by Lia.lia.
    reflexivity.
  * specialize (sum'def (fun y1 : int => nth (L_to_list a18 a2 0 1000) y1) i0 x).
    ...
    simpl.
    unfold a20.
    specialize (Map.set'def (havoc t1 a18 a4 i5) a i6 (shift a2 (x - 1))).
    intros H8;destruct H8 as (_ & H8).
    rewrite H8.
    specialize (havoc_access t1 a18 (shift a2 (x - 1)) a4  i5).
    ...
    unfold a.
    unfold global.
    specialize(separated_1 a2 a 1000 1 (x-1) 0).
    intros.
    apply H9.
    unfold separated.
    right.
...
* destruct offset'def as (H5 & _).
  rewrite H5 by apply H0.
  assert(H6: i1 = i5) by Lia.lia.
  rewrite H6.
  replace (i5 + i5 * rank)%Z with ((rank + 1) * i5)%Z by Lia.lia.
  replace i5 with (1000 ÷ i)%Z by apply H0.
  replace (1000 ÷ i * rank)%Z with (rank * (1000 ÷ i))%Z by Lia.lia.
  replace i with size by apply H0.
  rewrite H.
  rewrite H4.
  reflexivity.
...
rewrite H in h32.
rewrite H6 in h32.
replace i5 with (1000 ÷ i)%Z in h32 by apply H0.
rewrite Z.mul_comm.
rewrite Z.add_comm.
replace i with size in h32 by apply H0.
apply h32.
...
assert(H6: i1 = i5) by Lia.lia.
rewrite H6.
replace i5 with 1%Z;[ | symmetry; apply H0].
replace (1 + 1 * rank)%Z with ((rank + 1) * 1)%Z by Lia.lia.
replace (1 * rank)%Z with (rank * 1)%Z by Lia.lia.
rewrite H4.
reflexivity.
...
* Lia.lia.
* destruct h58.
  destruct active'def as (H2 & _).
  rewrite H2 by apply H0.
  Lia.lia.
  destruct active'def as (_ & H2).
  rewrite H2 by apply H0.
  Lia.lia.
* simpl. auto.
* split.
