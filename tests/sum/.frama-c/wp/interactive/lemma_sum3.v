...
intros.
assert (H4:(i1 = x \/ i1 < x)%Z) by Lia.lia.
destruct H4.
- rewrite H4.
  rewrite (Q_sum1 _ _ x x).
  rewrite Int.Unit_def_r.
  reflexivity.
  reflexivity.
- replace x with (1 + (x-1))%Z by Lia.lia.
  rewrite <- (Q_sum2 t a i (1 + (x -1))) by Lia.lia.
  rewrite <- (Q_sum2 t a i1 (1 + (x -1))) by Lia.lia.
  replace (- (1) + (1 + (x - 1)))%Z with (x-1)%Z by Lia.lia.
  rewrite <- (H2 (x-1)%Z) by Lia.lia.
  rewrite Z.add_assoc.
  reflexivity.
Qed.
