...
(* Why3 goal *)
Theorem wp_goal :
  let a := global 1932%Z in
  let a1 := global 1921%Z in
  let a2 := shift a1 0%Z in
  let a3 := global 1922%Z in
  ...
  let a6 := L_get_type c5 in
  let a7 := getNext a6 in
  let a8 := getFirst a6 in
  let a9 := L_get_type c3 in
  ...
  let x := (i2 * (1%Z + i3)%Z)%Z in
  let x1 := t1 a in
  let a14 := getNext a9 in
  let a15 := L_get_type c7 in
  let a16 := L_get_type c in
  let a17 := getFirst a16 in
  let a18 := L_get_type c8 in
  let a19 := getFirst a18 in
  let a20 := havoc t3 a11 a4 1000%Z in
  let a21 := havoc t1 a12 (shift a 0%Z) 1%Z in
  (rank = 0%Z) -> ((t a) = 0%Z) -> ((getNext a5) = a7) ->
  ((getNext (split (getFirst a5) i4)) = a8) ->
  ((getNext (split (getFirst (L_get_type c4)) i3)) = a10) ->
  ((f a13) = a5) -> (0%Z < i4)%Z -> (0%Z < i3)%Z -> (0%Z < size)%Z ->
  (i2 <= i5)%Z -> (i <= i4)%Z -> (i3 < i)%Z -> (i3 < size)%Z ->
  (0%Z <= i2)%Z -> (i5 <= i2)%Z -> (0%Z <= i1)%Z -> (i4 <= i)%Z ->
  (i3 <= i)%Z -> (1000%Z <= i1)%Z -> (i1 <= 1000%Z)%Z -> (x <= 1000%Z)%Z ->
  ...
  P_set_type c6 (simpl a15) -> P_set_type c4 a7 ->
  P_set_type c1 (getNext a16) -> P_set_type c (assoc (L_get_type c2)) ->
  isSkip (simpl a8) -> isMessageforIntRecv a17 i3 1%Z 1%Z ->
  P_set_type c2 (Seq (simpl a10) a14) -> is_sint32 (L_sum a12 a2 0%Z i2) ->
  is_sint32 (L_sum a12 a2 0%Z (i3 * i5)%Z) -> predIntBroadcast a19 a13 ->
  isforIntGhostBroadcast a19 0%Z 1000%Z (L_to_list a20 a4 0%Z 1000%Z) ->
  countiIntBroadcast a19 (getNext a18) a15 a13 ->
  ...
  (forall (i6:Numbers.BinNums.Z), ~ (i4 = i) -> (0%Z <= i6)%Z ->
   (i6 < i5)%Z -> ((i6 + (i4 * i5)%Z)%Z <= 999%Z)%Z) ->
...
unfold a18 in h46.
unfold a17 in h46.
erewrite Q_link in h46 by apply h37.
erewrite Q_link in h46 by apply h40.
rewrite <- h6 in h46.
erewrite Q_link in h46 by apply h35.
rewrite <- h4 in h46.
rewrite <- h7 in h46.
unfold f in h46.
simpl in h46.
...
replace (1000 ÷ i * i3)%Z with (i3 * (1000 ÷ i))%Z by Lia.lia.
replace (1000 ÷ i * (1 + i3))%Z with ((i3 + 1) * (1000 ÷ i))%Z by Lia.lia.
...
rewrite Z.mul_comm.
replace (i3 + 1)%Z with (1 + i3)%Z by Lia.lia.
...
replace (1 * i3)%Z with (i3 * 1)%Z by Lia.lia.
replace (1 * (1 + i3))%Z with ((i3 + 1) * 1)%Z by Lia.lia.
rewrite H4.
reflexivity.
